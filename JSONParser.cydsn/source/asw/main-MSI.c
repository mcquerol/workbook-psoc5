...
TASK(tsk_json)
{
    ...
    uint8_t read;
    ringbuffer_read(&rb,&read);
    UART_LOG_PutChar('-');
    UART_LOG_PutChar('\n');
    UART_LOG_PutChar(read);
    
    //TODO FIX output of ^^^ it is not showing in uart properly..must debug
...
ISR2(isr_uart_rx)
{
    ...
    while(UART_LOG_GetRxBufferSize() != 0)
    {
        ...
        data = (uint8_t)UART_LOG_GetByte();
        //UART_LOG_PutChar(data);
        if(data == '0')
        {
