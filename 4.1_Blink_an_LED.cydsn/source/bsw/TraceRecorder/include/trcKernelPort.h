/*******************************************************************************
 * Trace Recorder Library for Tracealyzer v4.1.0
...
/********** ERIKA TRACING DEFINES HERE *************/
//#define __OO_configUSE_TRACE_FACILITY 1
#define __OO_configUSE_TRACE_TASK_SWITCH 1
#define __OO_configUSE_TRACE_EVENTS		 1
#define __OO_configUSE_TRACE_ALARMS		 1
#define __OO_configUSE_TRACE_RESOURCE	 1
...
/* Added mainly for our internal testing. This makes it easier to create test applications that 
   runs on multiple FreeRTOS versions. */
	...
	#define TickType EE_UREG //ERIKA OD Changes TX update
#endif

/* Called by TzCtrl task periodically (Normally every 100 ms) */
void prvCheckRecorderStatus(void);//ERIKA_Changes_Tx Updated-removed static
...
//TaskType prvTraceGetCurrentTaskId()
...
#define TRACE_TICK_RATE_HZ 1000 
#define TRACE_CPU_CLOCK_HZ BCLK__BUS_CLK__HZ

#define TRACE_GET_CURRENT_TASK() prvTraceGetCurrentTaskId()
...
/*
 * ERIKA Changes- removed the below queue functions corresponding to TX version update for the ring buffer implementation.
 * In general the implementation for these functions is specific to FreeRTOS which is not taken over to ERIKA.
*/
/*
...
#define PSF_EVENT_CANCEL_ALARM                              0xA0
#define PSF_EVENT_SET_ABS_ALARM                             0xA1
#define PSF_EVENT_SET_REL_ALARM                             0xA2
#define PSF_EVENT_GET_ALARM                                 0xA3

#define PSF_EVENT_ALARM_CREATE                              0x14
...
#define PSF_EVENT_SET_EVENT								    0xC9
#define PSF_EVENT_GET_EVENT							        0xCA
#define PSF_EVENT_CLEAR_EVENT                               0xCB
#define PSF_EVENT_ACTION_ACTIVATE_TASK  					0xAA
#define PSF_EVENT_WAIT_EVENT    							0xCD
#define PSF_EVENT_TASK_NOTIFY_WAIT_BLOCK					0xCE
#define PSF_EVENT_TASK_NOTIFY_WAIT_FAILED					0xCF
#define PSF_EVENT_SET_EVENT_FROM_ISR						0xD0
...
#define PSF_EVENT_RESOURCE_CREATE 0x13 //same as mutex create
#define PSF_EVENT_GET_RESOURCE 0x62    //mutex/semaphore take
#define PSF_EVENT_RELEASE_RESOURCE 0x52 //same as sem/mutex give
...
#define traceTASK_INCREMENT_TICK() \
	...
	prvTraceStoreEvent1(PSF_EVENT_NEW_TIME, (uint32_t)uiTraceTickCount); 
#endif /* TRC_CFG_FREERTOS_VERSION <= TRC_FREERTOS_VERSION_7_4 */

/* Called on each task-switch */
#undef traceTASK_SWITCHED_IN
#if (__OO_configUSE_TRACE_TASK_SWITCH == 1)
#define traceTASK_SWITCHED_IN(taskID, taskPriority) \
			prvTraceStoreEvent2(PSF_EVENT_TASK_ACTIVATE, (uint32_t)taskID+TASK_ID_OFFSET, taskPriority); \
...
#undef traceMOVED_TASK_TO_READY_STATE
#define traceMOVED_TASK_TO_READY_STATE( taskID ) \
		prvTraceStoreEvent1(PSF_EVENT_TASK_READY, taskID+TASK_ID_OFFSET);
#endif

#undef traceTASK_CREATE
#if TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_9_0_0
#define traceTASK_CREATE(taskID, taskName, taskPriority) \
	{ \
		prvTraceSaveSymbol(taskID+TASK_ID_OFFSET, taskName); \
		prvTraceSaveObjectData(taskID+TASK_ID_OFFSET, taskPriority); \
		prvTraceStoreStringEvent(1, PSF_EVENT_OBJ_NAME, taskName, taskID+TASK_ID_OFFSET); \
				prvTraceStoreEvent2(PSF_EVENT_TASK_CREATE, (uint32_t)taskID+TASK_ID_OFFSET, taskPriority); \
	}
#else /* TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_9_0_0 */
#define traceTASK_CREATE(pxNewTCB) \
	if (pxNewTCB != NULL) \
...
#undef traceALARM_CREATE
#if (__OO_configUSE_TRACE_ALARMS == 1)
#define traceALARM_CREATE(alarmID, alarmName, ticks) \
	{ \
		prvTraceSaveSymbol(alarmID+ALARM_ID_OFFSET, alarmName); \
		prvTraceStoreStringEvent(1, PSF_EVENT_OBJ_NAME, alarmName, alarmID+ALARM_ID_OFFSET); \
		prvTraceStoreEvent2(PSF_EVENT_ALARM_CREATE, (uint32_t)alarmID+ALARM_ID_OFFSET, ticks); \
	}
#else
#define traceALARM_CREATE(alarmID, alarmName, ticks)  
#endif

#undef traceRESOURCE_CREATE
#if (__OO_configUSE_TRACE_RESOURCE == 1)
#define traceRESOURCE_CREATE(resID, resName, junk) \
	{ \
		prvTraceSaveSymbol(resID+RESOURCE_ID_OFFSET, resName); \
		prvTraceStoreStringEvent(1, PSF_EVENT_OBJ_NAME, resName, resID+RESOURCE_ID_OFFSET); \
		prvTraceStoreEvent2(PSF_EVENT_RESOURCE_CREATE, (uint32_t)resID+RESOURCE_ID_OFFSET, junk); \
	}
#else
    #define traceRESOURCE_CREATE(resID, resName, junk)
#endif
...
#undef traceQUEUE_CREATE
#define traceQUEUE_CREATE( pxNewQueue )\
        prvTraceSaveSymbol(pxNewQueue, pxNewQueue->pcQueueName); \
		prvTraceStoreStringEvent(1, PSF_EVENT_OBJ_NAME, pxNewQueue->pcQueueName, pxNewQueue); \
			switch (pxNewQueue->ucQueueType) \
			{ \
				case queueQUEUE_TYPE_BASE: \
					prvTraceStoreEvent2(PSF_EVENT_QUEUE_CREATE, (uint32_t)pxNewQueue, pxNewQueue->size); \
					break; \
...
/* Called when a message is sent to a queue */	/* CS IS NEW ! 
 * ERIKA Changes-The send definition has been changed for ring buffer implementation in ERIKA.
 * ERIKA OS Changes, removed xCopyPosition
 * The trace recorder update on queue implementtaion is also removed
*/
#undef traceQUEUE_SEND
#define traceQUEUE_SEND( pxQueue ) \
			switch (pxQueue->ucQueueType) \
...
#define traceQUEUE_SEND_FAILED( pxQueue ) \
			...
			{ \
				case queueQUEUE_TYPE_BASE: \
					prvTraceStoreEvent2(PSF_EVENT_QUEUE_SEND_FAILED, (uint32_t)pxQueue, pxQueue->uxMessagesWaiting); \
					break; \
				case queueQUEUE_TYPE_BINARY_SEMAPHORE: \
				case queueQUEUE_TYPE_COUNTING_SEMAPHORE: \
...
#undef traceALARM_SET_REL_ALARM
#if (__OO_configUSE_TRACE_ALARMS == 1)
#define traceALARM_SET_REL_ALARM(alarmID, ticks, cycle) \
		prvTraceStoreEvent2(PSF_EVENT_SET_REL_ALARM, (uint32_t)alarmID+ALARM_ID_OFFSET, cycle);
#else
    #define traceALARM_SET_REL_ALARM(alarmID, ticks, cycle)
#endif

#undef traceALARM_SET_ABS_ALARM
#if (__OO_configUSE_TRACE_ALARMS == 1)
#define traceALARM_SET_ABS_ALARM(alarmID, ticks, cycle) \
		prvTraceStoreEvent3(PSF_EVENT_SET_ABS_ALARM, (uint32_t)alarmID+ALARM_ID_OFFSET, ticks, cycle);
#else
  #define traceALARM_SET_ABS_ALARM(alarmID, ticks, cycle)  
#endif

#undef traceALARM_CANCEL_ALARM
#if (__OO_configUSE_TRACE_ALARMS == 1)
#define traceALARM_CANCEL_ALARM(alarmID) \
		prvTraceStoreEvent1(PSF_EVENT_CANCEL_ALARM, (uint32_t)alarmID+ALARM_ID_OFFSET);
#else
#define traceALARM_CANCEL_ALARM(alarmID)
#endif

#undef traceALARM_GET_ALARM
#if (__OO_configUSE_TRACE_ALARMS == 1)
#define traceALARM_GET_ALARM(alarmID, ticks) \
		prvTraceStoreEvent2(PSF_EVENT_GET_ALARM, (uint32_t)alarmID+ALARM_ID_OFFSET, ticks);
#else
#define traceALARM_GET_ALARM(alarmID, ticks)    
#endif

#undef traceRESOURCE_GET_RESOURCE
#if (__OO_configUSE_TRACE_RESOURCE == 1)
#define traceRESOURCE_GET_RESOURCE(resID, status) \
		prvTraceStoreEvent2(PSF_EVENT_GET_RESOURCE, (uint32_t)resID+RESOURCE_ID_OFFSET, status);
#else
#define traceRESOURCE_GET_RESOURCE(resID, status)    
#endif

#undef traceRESOURCE_RELEASE_RESOURCE
#if (__OO_configUSE_TRACE_RESOURCE == 1)
#define traceRESOURCE_RELEASE_RESOURCE(resID, status) \
		prvTraceStoreEvent2(PSF_EVENT_RELEASE_RESOURCE, (uint32_t)resID+RESOURCE_ID_OFFSET, status);
#else
#define traceRESOURCE_RELEASE_RESOURCE(resID, status)
#endif
...
/* Called when a message is received from a queue 
 * ERIKA Changes-The receive failed definition has been changed for ring buffer implementation in ERIKA. 
 * Removed the JustPeeking part of the implementation.
 * The trace recorder update on queue implementtaion is also removed.
*/
#undef traceQUEUE_RECEIVE
#define traceQUEUE_RECEIVE( pxQueue ) \
...
/* Called when a receive operation on a queue fails (timeout)
 * ERIKA Changes-The receive failed definition has been changed for ring buffer implementation in ERIKA. 
 * Removed the JusPeeking part of the implementation
 * The trace recorder update on queue implementtaion is also removed
*/
#undef traceQUEUE_RECEIVE_FAILED
#define traceQUEUE_RECEIVE_FAILED( pxQueue ) \
			...
			{ \
				case queueQUEUE_TYPE_BASE: \
					prvTraceStoreEvent3(PSF_EVENT_QUEUE_RECEIVE_FAILED, (uint32_t)pxQueue, xTicksToWait, pxQueue->uxMessagesWaiting); \
					break; \
				case queueQUEUE_TYPE_BINARY_SEMAPHORE: \
				case queueQUEUE_TYPE_COUNTING_SEMAPHORE: \
...
#undef traceTIMER_EXPIRED
#define traceTIMER_EXPIRED(tmr) \
	if (tmr->pxCallbackFunction == NULL)\
        prvTraceStoreEvent2(PSF_EVENT_TIMER_EXPIRED,(uint32_t)tmr,(uint32_t)tmr->uxTimerNumber);\
            else \
			    prvTraceStoreEvent2(PSF_EVENT_TIMER_EXPIRED, (uint32_t)tmr->pxCallbackFunction, (uint32_t)tmr->uxTimerNumber);
...
#undef traceTASK_GET_EVENT
#if (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_9_0_0)
#define traceTASK_GET_EVENT(taskID, event) \
	prvTraceStoreEvent2(PSF_EVENT_GET_EVENT, (uint32_t)taskID+TASK_ID_OFFSET, event);

#else /* TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_9_0_0 */
#define traceTASK_GET_EVENT() \
	if (TRACE_GET_TASK_FILTER(TRACE_GET_CURRENT_TASK()) & CurrentFilterMask){ \
		if (pxCurrentTCB->eNotifyState == eNotified) \
			prvTraceStoreEvent2(traceTASK_GET_EVENT, (uint32_t)pxCurrentTCB, xTicksToWait); \
		else \
			prvTraceStoreEvent2(PSF_EVENT_TASK_NOTIFY_TAKE_FAILED, (uint32_t)pxCurrentTCB, xTicksToWait);}
#endif /* TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_9_0_0 */

#undef traceTASK_CLEAR_EVENT
#if (__OO_configUSE_TRACE_EVENTS == 1)
#define traceTASK_CLEAR_EVENT(taskID, event) \
		prvTraceStoreEvent2(PSF_EVENT_CLEAR_EVENT, (uint32_t)taskID+TASK_ID_OFFSET, event);
#else
#define traceTASK_CLEAR_EVENT(taskID, event)    
#endif

#undef traceALARM_ACTION_ACTIVATE_TASK
#if (__OO_configUSE_TRACE_ALARMS == 1)
#define traceALARM_ACTION_ACTIVATE_TASK(alarmID, taskID) \
		prvTraceStoreEvent2(PSF_EVENT_ACTION_ACTIVATE_TASK, (uint32_t)alarmID+ALARM_ID_OFFSET, (uint32_t)taskID+TASK_ID_OFFSET);
#else
#define traceALARM_ACTION_ACTIVATE_TASK(alarmID, taskID)    
#endif


#undef traceTASK_WAIT_EVENT
#if (__OO_configUSE_TRACE_EVENTS == 1)
#define traceTASK_WAIT_EVENT(taskID, event) \
			prvTraceStoreEvent2(PSF_EVENT_WAIT_EVENT, (uint32_t)taskID+TASK_ID_OFFSET, event); 
#else
traceTASK_WAIT_EVENT(taskID, event)
#endif
...
#undef traceTASK_SET_EVENT
#if (__OO_configUSE_TRACE_EVENTS == 1)
#define traceTASK_SET_EVENT(taskID, event) \
			prvTraceStoreEvent2(PSF_EVENT_SET_EVENT, (uint32_t)taskID+TASK_ID_OFFSET, event);
#else
#define traceTASK_SET_EVENT(taskID, event)    
#endif    

#undef traceTASK_SET_EVENT_FROM_ISR
#if (__OO_configUSE_TRACE_EVENTS == 1)
#define traceTASK_SET_EVENT_FROM_ISR(taskID, event) \
		prvTraceStoreEvent1(PSF_EVENT_SET_EVENT_FROM_ISR, (uint32_t)taskID+TASK_ID_OFFSET, event);
#else
#define traceTASK_SET_EVENT_FROM_ISR(taskID, event)    
#endif  
