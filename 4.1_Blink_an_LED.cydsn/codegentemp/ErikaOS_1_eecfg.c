 ...
 **************************************************************************/
    ...
    #define TASK_3_STACK_SIZE 100/4 // size = 100 bytes
    int EE_cortex_mx_stack_3[TASK_3_STACK_SIZE];	/* Task 3 (tsk_SampleTask) */
    #endif
    #if ((1 == 2) && (EE_MAX_TASK > 3))
    #define TASK_4_STACK_SIZE 0/4 // size = 0 bytes
    int EE_cortex_mx_stack_4[TASK_4_STACK_SIZE];	/* Task 4 (tsk_background) */
    #endif
    #if ((1 == 2) && (EE_MAX_TASK > 4))
    #define TASK_5_STACK_SIZE 0/4 // size = 0 bytes
    int EE_cortex_mx_stack_5[TASK_5_STACK_SIZE];	/* Task 5 (tsk_background) */
    #endif
    ...
    struct EE_TOS EE_cortex_mx_system_tos[EE_CORTEX_MX_SYSTEM_TOS_SIZE] = {
        ...
        #endif
        #ifdef TASK_3_STACK_SIZE
        ,{(EE_ADDR)(&EE_cortex_mx_stack_3[(TASK_3_STACK_SIZE) - CORTEX_MX_INIT_TOS_OFFSET])} 	/* tsk_SampleTask*/
        #endif
        #ifdef TASK_4_STACK_SIZE
        ,{(EE_ADDR)(&EE_cortex_mx_stack_4[(TASK_4_STACK_SIZE) - CORTEX_MX_INIT_TOS_OFFSET])}    /* tsk_background*/
        #endif
        #ifdef TASK_5_STACK_SIZE
        ,{(EE_ADDR)(&EE_cortex_mx_stack_5[(TASK_5_STACK_SIZE) - CORTEX_MX_INIT_TOS_OFFSET])}    /* tsk_background*/
        #endif
        #ifdef TASK_6_STACK_SIZE
 ...
 **************************************************************************/
    ...
    #endif
    #if EE_MAX_TASK > 2
        ,&EE_oo_thread_stub      /* thread tsk_SampleTask */
    #endif
    #if EE_MAX_TASK > 3
        ,&EE_oo_thread_stub      /* thread tsk_background */
    #endif
 ...
 **************************************************************************/
    ...
    const EE_oo_counter_object_ROM_type   EE_oo_counter_object_ROM[EE_COUNTER_OBJECTS_ROM_SIZE] = {
        #if EE_MAX_ALARM > 0
        {0, alrm_SampleAlarm, EE_ALARM }
        #endif
        #if EE_MAX_ALARM > 1
        ,{0, alrm_1000ms, EE_ALARM }
        #endif
        #if EE_MAX_ALARM > 2
        ,{0, alrm_tick, EE_ALARM }
        #endif
        #if EE_MAX_ALARM > 3
        ,{0, Alarm_4, EE_ALARM }
        #endif
